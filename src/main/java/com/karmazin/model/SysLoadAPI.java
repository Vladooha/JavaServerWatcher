 ...
 */
public class SysLoadAPI implements Runnable {
    private static final LoggerAPI logger = new LoggerAPI(SysLoadAPI.class.getName());

    private static final int cpuOverload = 90;
    private static final double memPercLack = 0.05;

    private static SysLoadAPI sysLoadAPI;
    ...
    public static SysLoadAPI getSysLoadAPI() {
        if (sysLoadAPI == null) {
            sysLoadAPI = new SysLoadAPI();
        }
    ...
    private SysLoadAPI() {
        sleep = ConfigWrapper.getProcessDelay();
    ...
    /*
    public SysLoadAPI(int PID) {
        this.PID = PID;
        t = new Thread(this);
        logger.log(Level.INFO, "New watcher for " + PID + ": with sleep " + sleep + ": " + t);
        t.start();
    }
    public SysLoadAPI(int PID, int sleep) {
        this.PID = PID;
        this.sleep = sleep;
        t = new Thread(this);
        logger.log(Level.INFO, "New watcher for " + PID + ": with sleep " + sleep + ": " + t);
            ...
            while (running.get()) {
                try {
                    ...
                    try (BufferedReader inputCpu = new BufferedReader(new InputStreamReader(pCpu.getInputStream()));
                        ...
                        //System.err.println("CPU load: " + getCpuLoad() + ", free mem %: " + getFreeMemPerc());
                        ...
                        if (isOverloaded()) {
                            ...
                            if (loadedSession > 10) {
                                ...
                                if (System.currentTimeMillis() - time > 1200000) {
                                    ...
                                    try {
                                        EmailAPI email = new EmailAPI("javaexamplesas", "Qwerty1337");
                                        email.localOverloadMessage(ConfigWrapper.getEmail(),
...
//                    running.set(false);
    ...
    public static double getFreeMemPerc() {
        return (double)freeMem / totalMem * 100;
    }
}
