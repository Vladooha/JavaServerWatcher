...
import com.karmazin.model.ConfigWrapper;
...
public class SettingsScreen {
    ...
    void initialize() {
        // View pre-set
        settingProfileLabel.setText("Профиль (" + ConfigWrapper.getLogin() + ")");

        settingMailTextField.setText(ConfigWrapper.getEmail());

        settingPingDelayTextField.setText(ConfigWrapper.getPingDelay() + "");
        settingPingFaultsTextField.setText(ConfigWrapper.getPingFaults() + "");
        settingPingEmailTextField.setText(ConfigWrapper.getPingEmail() + "");
        settingHttpDelayTextField.setText(ConfigWrapper.getHttpDelay() + "");

        settingProcWaitTextField.setText(ConfigWrapper.getProcessDelay() + "");
        ...
        settingAcceptButton.setOnAction((event) -> {
            ...
            ConfigWrapper.setEmail(email);

            if (pass.length() != 0) {
                UserAPI.updatePassword(ConfigWrapper.getLogin(), pass);
            }

            ConfigWrapper.setPingDelay(pingDelay);
            ConfigWrapper.setPingFaults(pingFaults);
            ConfigWrapper.setPingEmail(pingEmail);
            ConfigWrapper.setHttpDelay(httpDelay);
            ConfigWrapper.setProcessDelay(procWait);
